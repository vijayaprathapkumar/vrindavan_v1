...
const getCustomers = async (req, res) => {
    const { page = 1, limit = 10, locality, status, searchTerm, sortField, sortOrder, } = req.query;
    const validPage = Math.max(1, Number(page));
    const validLimit = Math.min(Math.max(1, Number(limit)), 100);
    try {
        const { customers, total, statusCount } = await (0, customerModel_1.getAllCustomers)(validPage, validLimit, locality?.toString(), status?.toString(), searchTerm?.toString(), sortField?.toString(), sortOrder?.toString());
