...
const fetchFeaturedCategories = async (req, res) => {
    ...
    try {
        ...
        return res
            .status(500)
            .json((0, responseHandler_1.createResponse)(500, "Failed to fetch featured categories."));
    }
};
...
const addFeaturedCategory = async (req, res) => {
    ...
    if (!category_id || typeof status === "undefined") {
        return res
            .status(400)
            .json((0, responseHandler_1.createResponse)(400, "Missing required fields: category_id and status are mandatory."));
    }
    try {
        const newCategory = await (0, featureProductModel_1.createFeaturedCategory)({
            category_id,
            sub_category_id,
            status,
            category_type,
        ...
        return res
            .status(201)
            .json((0, responseHandler_1.createResponse)(201, "Featured category added successfully.", newCategory));
    }
    catch (error) {
        console.error("Error adding featured category:", error);
        return res
            .status(500)
            .json((0, responseHandler_1.createResponse)(500, "Failed to add featured category."));
    }
};
