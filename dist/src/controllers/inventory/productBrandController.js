...
const fetchAllBrands = async (req, res) => {
    try {
        ...
        const brands = await (0, productBrandModel_1.getAllBrands)(searchTerm, limit, offset, sortField, sortOrder);
        const totalCount = await (0, productBrandModel_1.getBrandsCount)(searchTerm);
        if (!brands || brands.length === 0 || totalCount === 0) {
            return res
                .status(404)
                .json((0, responseHandler_1.createResponse)(404, "No product brands found"));
        }
...
const fetchBrandById = async (req, res) => {
    ...
    if (isNaN(brandId)) {
        return res
            .status(400)
            .json((0, responseHandler_1.createResponse)(400, "Invalid product brand ID"));
    }
    try {
        ...
        if (brand) {
            ...
            return res
                .status(404)
                .json((0, responseHandler_1.createResponse)(404, "Product brand not found"));
        }
    }
    catch (error) {
