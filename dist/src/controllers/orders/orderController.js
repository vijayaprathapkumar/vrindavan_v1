...
const getOrder = async (req, res) => {
    const { id } = req.params;
    try {
        const order = await (0, orderModel_1.getOrderById)(parseInt(id));
        if (!order) {
...
const createOrder = async (req, res) => {
    try {
        const newOrder = await (0, orderModel_1.createOrderInDb)(req.body);
...
const updateOrder = async (req, res) => {
    const { id } = req.params;
    try {
        const updatedOrder = await (0, orderModel_1.updateOrderInDb)(parseInt(id), req.body);
...
const deleteOrder = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await (0, orderModel_1.deleteOrderInDb)(parseInt(id));
