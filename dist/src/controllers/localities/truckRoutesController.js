...
const getTruckRoutes = async (req, res) => {
    const { page = 1, limit = 10, searchTerm = "", sortField = "", sortOrder = "", } = req.query;
    const validLimit = Number(limit) > 0 ? Number(limit) : 10;
    try {
        const { routes, totalRecords } = await (0, truckRoutesModel_1.getAllTruckRoutes)(Number(page), validLimit, searchTerm.toString(), sortField.toString(), sortOrder.toString());
