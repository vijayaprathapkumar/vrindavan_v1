...
const getAllLocalities = async (page, limit, searchTerm, sortField, sortOrder) => {
  ...
  `;
    ...
    const [localities] = await databaseConnection_1.db.promise().query(localitiesQuery, params);
    const countQuery = `
...
const createLocality = async (routeId, hubId, name, address, googleAddress, latitude, longitude, city, active, deliveryBoyId) => {
    const connection = await databaseConnection_1.db.promise().getConnection();
    try {
        await connection.beginTransaction();
        const [localityResult] = await connection.query(`INSERT INTO localities 
      ...
      VALUES
        ...
        await connection.query(`INSERT INTO locality_delivery_boys 
        (locality_id, delivery_boy_id, created_at)
      VALUES
        (?, ?, NOW())`, [localityId, deliveryBoyId]);
...
const updateLocalityById = async (id, routeId, hubId, name, address, googleAddress, latitude, longitude, city, active, deliveryBoyId) => {
    const connection = await databaseConnection_1.db.promise().getConnection();
    try {
        await connection.beginTransaction();
        await connection.query(`UPDATE localities 
       ...
       WHERE id = ?`, [
        ...
        const [rows] = await connection.query(`SELECT id FROM locality_delivery_boys WHERE locality_id = ?`, [id]);
        if (rows.length > 0) {
            await connection.query(`UPDATE locality_delivery_boys 
         SET delivery_boy_id = ?, updated_at = NOW() 
         WHERE locality_id = ?`, [deliveryBoyId, id]);
        }
        else {
            await connection.query(`INSERT INTO locality_delivery_boys 
         (locality_id, delivery_boy_id, created_at) 
         VALUES (?, ?, NOW())`, [id, deliveryBoyId]);
        }
        await connection.commit();
    }
    catch (error) {
...
const deleteLocalityById = async (id) => {
    await databaseConnection_1.db
        .promise()
        .query("DELETE FROM localities WHERE id = ?", [id]);
};
