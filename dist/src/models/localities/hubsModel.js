...
const createHub = async (routeId, name, otherDetails, active) => {
    await databaseConnection_1.db
        .promise()
        .query("INSERT INTO hubs (route_id, name, other_details, active) VALUES (?, ?, ?, ?)", [routeId, name, otherDetails, active]);
};
...
const getHubById = async (id) => {
    const [rows] = await databaseConnection_1.db
        .promise()
        .query("SELECT * FROM hubs WHERE id = ?", [id]);
    return rows;
};
...
const updateHubById = async (id, routeId, name, otherDetails, active) => {
    await databaseConnection_1.db
        .promise()
        .query("UPDATE hubs SET route_id = ?, name = ?, other_details = ?, active = ? WHERE id = ?", [routeId, name, otherDetails, active, id]);
};
