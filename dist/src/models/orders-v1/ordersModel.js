...
const getAllOrders = async (userId, page, limit, startDate, endDate) => {
  ...
  `;
    ...
    if (startDate)
        countParams.push(startDate.toISOString().slice(0, 19).replace('T', ' '));
    if (endDate)
        countParams.push(endDate.toISOString().slice(0, 19).replace('T', ' '));
    const [countRows] = await databaseConnection_1.db.promise().query(countQuery, countParams);
