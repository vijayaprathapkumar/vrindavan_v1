                    ...
                    if (quantityToOrder > 0) {
                        ...
                        if (!orderResult.success) {
                            if (orderResult.reason === "missing_product") {
                                ...
                                console.warn(`Subscription ID ${sub.id}: Insufficient wallet balance. Skipping.`);
                            }
                            else {
...
const createOrder = async (orderItem, quantityToOrder, currentDate) => {
    ...
    try {
        ...
        const hasSufficientBalance = await withTimeout(checkWalletBalance(user_id, totalAmount), 5000);
        if (!hasSufficientBalance) {
            console.warn(`User ${user_id} has insufficient wallet balance for product ${product_id}. Required: ${totalAmount}`);
...
const subcribtionsJob = () => {
    node_cron_1.default.schedule("30 21 * * *", async () => {
        ...
        try {
            ...
            const logMessage = `Subscription orders placed for date ${currentDate.toISOString().split("T")[0]} and placed on ${currentDate.toLocaleString()}`;
            const sqlQuery = `
