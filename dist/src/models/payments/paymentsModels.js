...
const handlePaymentsOrders = async (placeOrderData) => {
    ...
    try {
        const [paymentResult] = await databaseConnection_1.db.promise().query(paymentSql, paymentValues);
        if (paymentResult.affectedRows === 0) {
            throw new Error("Payment insertion failed.");
        }
        const orderId = paymentResult.insertId;
        // Fetch current wallet balance
        const [walletRows] = await databaseConnection_1.db.promise().query(`SELECT balance FROM wallet_balances WHERE user_id = ?`, [userId]);
        ...
        await logWalletTransaction(userId, orderId, beforeBalance, price, afterBalance, `Rs ${price} deducted for user ID ${userId}`);
...
const getAllPlaceOrdersUsers = async () => {
    ...
    `;
    const [placeOrderRows] = await databaseConnection_1.db.promise().query(query);
    ...
    try {
        ...
        for (const userId in ordersByUserId) {
            ...
            for (const order of userOrders) {
                ...
                await (0, exports.handlePaymentsOrders)(placeOrderPaymentData);
            }
        }
    }
    catch (error) {
