...
exports.deleteOrderInDb = exports.updateOrderInDb = exports.createOrderInDb = exports.getOrderById = exports.getAllOrders = void 0;
...
const getAllOrders = async () => {
    ...
    `);
        console.log('Fetched rows:', rows); // Log raw rows
        const orders = rows.reduce((acc, row) => {
            // Find if the order already exists in the accumulator
            let existingOrder = acc.find((order) => order.id === row.order_id);
            ...
            if (!existingOrder) {
                existingOrder = {
                    ...
                    route: {
                        id: row.route_id,
                        name: row.route_name,
                        active: row.route_active, // Add any other necessary fields
                    },
                    hub: {
                        id: row.hub_id,
                        name: row.hub_name,
            ...
            // If there are food items, add them to the existing order's foods array
            if (row.food_id) {
        ...
        console.log('Processed orders:', orders); // Log processed orders
        return { data: orders };
    }
    catch (error) {
...
const createOrderInDb = async (orderData) => {
    try {
        const [result] = await databaseConnection_1.db.promise().query(`
      INSERT INTO orders (
...
const updateOrderInDb = async (id, orderData) => {
    try {
        const [result] = await databaseConnection_1.db.promise().query(`
      UPDATE orders SET
        user_id = ?, order_type = ?, order_date = ?, route_id = ?, hub_id = ?, locality_id = ?,
...
const deleteOrderInDb = async (id) => {
    try {
        const [result] = await databaseConnection_1.db.promise().query(`
...
exports.deleteOrderInDb = deleteOrderInDb;
