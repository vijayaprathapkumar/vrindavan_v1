...
exports.getCalendarOneTimeOrdersModel = exports.getCalendarWiseOrdersModel = exports.cancelOneTimeOrderModel = exports.getUpcomingOrdersModel = exports.cancelOrder = exports.deletePlaceOrderById = exports.updateSubscriptionOrders = exports.updateOneTimeOrders = exports.getPlaceOrderById = exports.getAllOrdersWithOutUserId = exports.getAllOrders = void 0;
const databaseConnection_1 = require("../../config/databaseConnection");
const getAllOrders = async (userId, page, limit, startDate, endDate, searchTerm) => {
  ...
  `;
    ...
    const orderData = [
        ...new Map((placeOrderRows || []).map(({ order_id, food_order_id, price, quantity, food_id, name, discount_price, description, package_items_count, weight, unit, cgst, sgst, subscription_type, track_inventory, featured, deliverable, weightage, media_id, model_type, model_id, uuid, collection_name, media_name, file_name, mime_type, disk, conversions_disk, size, manipulations, custom_properties, responsive_images, order_column, original_url, ...rest }) => [
            order_id,
            {
                ...rest,
                food_orders: {
...
const getAllOrdersWithOutUserId = async (page, limit, startDate, endDate, searchTerm, routeId, hubId, localityId, productId, approveStatus, orderType, deliveryBoyId, walletFilter, sortField, sortOrder) => {
    const offset = (page - 1) * limit;
    const queryParams = [];
    let conditions = "";
    if (searchTerm) {
     ...
     OR o.id LIKE ?
        ...
        queryParams.push(`%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`);
    }
 ...
 LEFT JOIN locality_delivery_boys ldb ON o.locality_id = ldb.locality_id
  ...
  `;
    ...
    const [placeOrderRows] = await databaseConnection_1.db
        ...
        .query(query, queryParams);
    const orderData = [
        ...new Map((placeOrderRows || []).map(({ order_id, food_order_id, price, quantity, food_id, name, discount_price, description, package_items_count, weight, unit, cgst, sgst, subscription_type, track_inventory, featured, deliverable, weightage, product_type_id, category_id, subcategory_id, product_brand_id, media_id, model_type, model_id, uuid, collection_name, media_name, file_name, mime_type, disk, conversions_disk, size, manipulations, custom_properties, responsive_images, order_column, original_url, truck_route_id, truck_route_name, truck_route_active, truck_route_created_at, truck_route_updated_at, hub_id, hub_name, hub_details, hub_active, hub_created_at, hub_updated_at, locality_id, locality_route_id, locality_hub_id, locality_name, locality_address, google_address, latitude, longitude, city, locality_active, locality_created_at, locality_updated_at, delivery_addresses_description, delivery_addresses_address, delivery_addresses_latitude, delivery_addresses_longitude, delivery_addresses_house_no, delivery_addresses_complete_address, delivery_addresses_is_approve, delivery_addresses_is_default, delivery_addresses_user_id, delivery_addresses_locality_id, delivery_addresses_created_at, delivery_addresses_updated_at, payment_id, payment_price, payment_description, payment_user_id, payment_status, payment_method, payment_created_at, payment_updated_at, delivery_boy_id, delivery_boy_userId, delivery_boy_name, delivery_boy_mobile, delivery_boy_active, delivery_boy_cash_collection, delivery_boy_delivery_fee, delivery_boy_total_orders, delivery_boy_earning, delivery_boy_available, delivery_boy_addressPickup, delivery_boy_latitudePickup, delivery_boy_longitudePickup, delivery_boy_created_at, delivery_boy_updated_at, product_type_ids, product_type_name, product_type_weightage, product_type_active, product_type_created_at, product_type_updated_at, user_name, user_email, user_phone, user_delivery_priority, user_status, user_is_deactivated, user_is_deactivated_at, user_created_at, user_updated_at, order_status_id, order_status, order_status_created_at, order_status_updated_at, order_payment_id, ...rest }) => [
            order_id,
            {
                ...rest,
                order_id,
...
exports.getAllOrdersWithOutUserId = getAllOrdersWithOutUserId;
const getPlaceOrderById = async (orderId, searchTerm) => {
    let query = `
    SELECT 
    o.id AS order_id, 
    o.user_id, 
...
// Update Order
const updateOneTimeOrders = async (orderId, newQuantity) => {
    ...
    try {
        ...
        const [orderRows] = await connection.query(getOrderSql, [orderId]);
...
exports.updateOneTimeOrders = updateOneTimeOrders;
const updateSubscriptionOrders = async (subscriptionId, quantity, orderDate) => {
    try {
        // Fetch subscription items from the database
        const [subscriptionItems] = await databaseConnection_1.db
            .promise()
            .query(`SELECT user_id, product_id, monday_qty, tuesday_qty, wednesday_qty, 
        ...
        const [updateResult] = await databaseConnection_1.db.promise().query(`UPDATE subscription_quantity_changes 
       SET quantity = ? 
       WHERE user_subscription_id = ? AND order_date = ?`, [quantity, subscriptionId, orderDate]);
        // If no rows were affected, insert the new data
        if (updateResult.affectedRows === 0) {
            await databaseConnection_1.db.promise().query(`INSERT INTO subscription_quantity_changes (
          user_subscription_id, order_type, user_id, product_id, quantity, order_date
        ) VALUES (?, ?, ?, ?, ?, ?)`, [subscriptionId, 2, user_id, product_id, quantity, orderDate]);
        }
        if (orderDate) {
...
exports.updateSubscriptionOrders = updateSubscriptionOrders;
const deletePlaceOrderById = async (id) => {
  ...
  `;
    ...
    WHERE id = ?;
  `;
    await databaseConnection_1.db.promise().query(updateSubscriptionSql, [id]);
    await databaseConnection_1.db.promise().query(deleteFoodOrderSql, [id]);
...
const cancelOrder = async (subscriptionId, cancelOrderDate, reason, otherReason) => {
    try {
        ...
        const [userRow] = await databaseConnection_1.db
            .promise()
            .query(`SELECT user_id, start_date, end_date FROM user_subscriptions WHERE id = ?`, [subscriptionId]);
        if (userRow.length === 0) {
        ...
        await databaseConnection_1.db.promise().query(`DELETE FROM subscription_quantity_changes 
       WHERE user_subscription_id = ? AND order_date = ?`, [subscriptionId, formattedCancelOrderDate]);
        const [rows] = await databaseConnection_1.db.promise().query(`SELECT id FROM subscription_quantity_changes 
       WHERE user_subscription_id = ? AND cancel_order_date = ?`, [subscriptionId, formattedCancelOrderDate]);
        if (rows.length > 0) {
            await databaseConnection_1.db.promise().query(`UPDATE subscription_quantity_changes
         SET cancel_order = 1, reason = ?, other_reason = ?, user_id = ?
         WHERE id = ?`, [reason, otherReason || null, userId, rows[0].id]);
        }
        else {
            await databaseConnection_1.db.promise().query(`INSERT INTO subscription_quantity_changes (
...
const getUpcomingOrdersModel = (userId, currentDate) => {
  ...
  `;
    return new Promise((resolve, reject) => {
        ...
        databaseConnection_1.db.query(query, queryParams, (error, results) => {
            if (error) {
...
exports.getUpcomingOrdersModel = getUpcomingOrdersModel;
const cancelOneTimeOrderModel = (orderId) => {
    return new Promise(async (resolve, reject) => {
        const connection = await databaseConnection_1.db.promise().getConnection();
        try {
            await connection.beginTransaction();
            const deleteFoodOrdersQuery = "DELETE FROM food_orders WHERE order_id = ?";
            await connection.query(deleteFoodOrdersQuery, [orderId]);
            const deleteOrdersQuery = "DELETE FROM orders WHERE id = ?";
            const [result] = await connection.query(deleteOrdersQuery, [orderId]);
            await connection.commit();
            connection.release();
...
exports.cancelOneTimeOrderModel = cancelOneTimeOrderModel;
const getCalendarWiseOrdersModel = (userId, startDate, endDate) => {
...
ORDER BY c.calendar_date ASC;
  `;
    return new Promise((resolve, reject) => {
        databaseConnection_1.db.query(query, [formattedStartDate, formattedEndDate, userId], (error, results) => {
            if (error) {
                console.error("SQL Error:", error);
                return reject(error);
            }
...
exports.getCalendarWiseOrdersModel = getCalendarWiseOrdersModel;
const getCalendarOneTimeOrdersModel = (userId, startDate, endDate) => {
    const formattedStartDate = startDate.toISOString().split("T")[0];
    const formattedEndDate = endDate.toISOString().split("T")[0];
    const query = `
    WITH RECURSIVE calendar AS (
...
exports.getCalendarOneTimeOrdersModel = getCalendarOneTimeOrdersModel;
