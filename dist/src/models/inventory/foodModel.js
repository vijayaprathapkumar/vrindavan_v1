...
exports.updateStock = exports.deleteFood = exports.updateFood = exports.createFood = exports.getFoodById = exports.getAllFoods = void 0;
const databaseConnection_1 = require("../../config/databaseConnection");
const getAllFoods = async (filters, limit, offset, sortField, sortOrder) => {
    let query = `
    SELECT f.*, 
           m.id AS media_id,
           m.model_type,
...
END AS outOfStock
  ...
  `;
    ...
    if (filters.searchTerm) {
        conditions.push("(f.name LIKE ? OR f.unit LIKE ? OR f.id LIKE ? OR f.price LIKE ? OR f.discount_price LIKE ? OR f.weightage LIKE ?)");
        values.push(`%${filters.searchTerm}%`, `%${filters.searchTerm}%`, `%${filters.searchTerm}%`, `%${filters.searchTerm}%`, `%${filters.searchTerm}%`, `%${filters.searchTerm}%`);
    }
    ...
    ${sortField && validSortFields[sortField]
        ...
        : "CAST(f.weightage AS UNSIGNED)"} ${sortOrder === "desc" ? "DESC" : "ASC"}
  `;
    const countQuery = `
...
const getFoodById = async (id) => {
    const [rows] = await databaseConnection_1.db.promise().execute(`
      SELECT f.*, 
             m.id AS media_id,
             m.model_type,
             m.model_id,
...
const updateStock = async (foodId, amountChange, stockableType, description) => {
    try {
