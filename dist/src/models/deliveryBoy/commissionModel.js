...
const getAllDetailedCommissions = async (searchTerm = "", limit = 10, offset = 0, categoryId = "", sortField, sortOrder) => {
    const searchPattern = `%${searchTerm}%`;
    const categoryFilter = categoryId && categoryId !== "All" ? " AND c.id = ?" : "";
...
const updateCommission = async (commissionId, commissionValue) => {
  ...
  `;
    const [result] = await databaseConnection_1.db
        .promise()
        .query(query, [commissionValue, commissionId]);
