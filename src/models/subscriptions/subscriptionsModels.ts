...
export const addSubscriptionQuantityChangeModel = (
  ...
  return new Promise((resolve, reject) => {
    const changeData = {
      ...
      product_id,
      quantity,
      order_date: new Date(new Date().setDate(new Date().getDate() + 1)) || null,
  ...
  return new Promise((resolve, reject) => {
    let query = `
      SELECT user_subscriptions.*, 
             ...
             sqc.order_type AS subscription_quantity_order_type,
             sqc.user_id,
             sqc.product_id subscription_quantity_product_id,
