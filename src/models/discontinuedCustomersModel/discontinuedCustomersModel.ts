...
export const getDiscontinuedCustomers = async (
  limit: number,
  offset: number,
  sortField: string,
  sortOrder: "asc" | "desc",
...
const mapSubscriptionStatus = (searchTerm: string): number => {
  const subscriptionStatusMap: Record<string, number> = {
    Paused: 1,
    Active: 0,
  };

  const matchedKey = Object.keys(subscriptionStatusMap).find((key) =>
...
export const getDiscontinuedCustomersCount = async (): Promise<number> => {
  const query = `
