...
export const getWalletBalance = async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res.status(200).json(createResponse(200, "Wallet balance retrieved successfully", response)); 
    } catch (error) {
      console.error("Error retrieving wallet balance:", error);
      return res.status(500).json(createResponse(500, "Error retrieving wallet balance", error.message));
    }
  };
