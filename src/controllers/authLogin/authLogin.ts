...
export const requestOTP = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    db.query(
      "SELECT * FROM login_otp WHERE mobile_number = ?",
      [mobile_number],
      (err: Error | null, result: RowDataPacket[]) => {
        if (err) return res.status(500).json(createResponse(500, "Database error"));
...
export const verifyOTP = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    db.query(
      "SELECT * FROM login_otp WHERE mobile_number = ? AND otp_code = ? AND otp_expires_at > NOW()",
      [mobile_number, otp],
      (err: Error | null, result: RowDataPacket[]) => {
        if (err) return res.status(500).json(createResponse(500, "Database error"));
